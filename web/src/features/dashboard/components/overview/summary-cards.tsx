...
export function SummaryCards() {
  ...
  return (
    <div className='brand-card overflow-hidden rounded-2xl border border-[rgba(232,163,61,0.14)] shadow-xs'>
        ...
        <div className='flex flex-col gap-2.5 p-3 sm:gap-3 sm:p-5'>
          ...
          <StaggerContainer className='grid grid-cols-3 gap-1.5 sm:gap-3'>
            {items.map((it) => (
              <StaggerItem
                key={it.key}
                className='brand-card rounded-lg border border-[rgba(232,163,61,0.14)] px-2 py-1.5 sm:rounded-xl sm:p-3'
              >
                <StatCard
                  title={it.title}
                  value={it.value}
